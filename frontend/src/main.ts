...
async function loadRoute(): Promise<RouteData> {
  // Prefer /data (Vite serves public dir at root), fallback to legacy /public path
  const res = await fetch("/data/kimara_kivukoni_stops.json").catch(() =>
    fetch("/public/data/kimara_kivukoni_stops.json")
  );
...
function createStopMarker(stop: Stop) {
  return L.circleMarker([stop.latitute, stop.longtude], {
    ...
    fillColor: "#ffb703",
    fillOpacity: 0.9,
  }).bindTooltip(`<strong>${stop.stop_name}</strong><br/>ID: ${stop.stop_id}`, {
    direction: "top",
  });
}
...
async function init() {
  ...
  const stops: Stop[] = (data.stops || []).map((s: any) => ({
    ...
    distance_next_stop:
      s.distance_next_stop ?? s.distance_to_next ?? s.DistanceToNext ?? 0,
  }));
  ...
  const osmHot = L.tileLayer(
    "https://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png",
    {
      maxZoom: 20,
      attribution:
        "© OpenStreetMap contributors, Tiles style by Humanitarian OpenStreetMap Team hosted by OSM France",
    }
  );
  ...
  try {
    ...
    for (const p of rawPoints) {
      if (
        !dedup.length ||
        dedup[dedup.length - 1][0] !== p[0] ||
        dedup[dedup.length - 1][1] !== p[1]
      )
        dedup.push(p);
    }
    ...
    for (const orig of dedup) {
      ...
      for (const pp of polyPoints) {
        if (
          Math.abs(pp[0] - orig[0]) < 1e-9 &&
          Math.abs(pp[1] - orig[1]) < 1e-9
        ) {
          contained = true;
          break;
        }
  ...
  if (!ctrl) {
    ...
    ctrl.innerHTML = `
      <label>Speed: <span id='speed-val'>1.00x</span></label>
      <input id='speed-range' type='range' min='0.25' max='10' step='0.25' value='1' style='width:100%'/>
  ...
  const toggleStops = document.getElementById(
    "toggle-stops"
  ) as HTMLInputElement;
  function setStopsVisible(v: boolean) {
    for (const id in stopLabels) {
      const el = stopLabels[id].getElement();
      if (el) el.style.display = v ? "" : "none";
    }
  }
  toggleStops?.addEventListener("change", () =>
    setStopsVisible(toggleStops.checked)
  );
  ...
  function createBusState(
    ...
    const m = L.marker([lat, lng], {
      icon: createBusIcon(),
      title: `Bus ${id} (${direction})`,
    }).addTo(map);
    const lbl = L.marker([lat, lng], {
      interactive: false,
      icon: L.divIcon({
        className: "bus-occupancy-label",
        html: makeBusLabelHTML(capacity, onboard),
      }),
    }).addTo(map);
  ...
  function refreshBus(b: BusState) {
    ...
    b.label.setIcon(
      L.divIcon({
        className: "bus-occupancy-label",
        html: makeBusLabelHTML(b.capacity, b.onboard),
      })
    );
  }
  ...
  function updateStopCount(id: number, outbound: number, inbound?: number) {
    const el = document.querySelector(
      `div[data-stop='${id}'] div[data-stop-count='${id}']`
    );
    if (el) {
  ...
  function renderLegend(state?: string) {
    ...
    if (!el) return;
    el.innerHTML =
      `<div style='font-weight:600;margin-bottom:4px;min-width:150px;'>${data.route}</div>` +
      ...
      `<div>Avg wait: <strong>${totals.avgWaitMin.toFixed(
        2
      )} min</strong></div>` +
  ...
  function openStream(): EventSource {
    ...
    try {
      ...
      es = new EventSource(
        `/api/stream?speed=${encodeURIComponent(String(sp))}`
      );
    } catch {
      const sp = Number(speedRange?.value || "1");
      es = new EventSource(
        `http://localhost:8080/api/stream?speed=${encodeURIComponent(
          String(sp)
        )}`
      );
    }
        ...
        if (typeof d.boarded === "number") {
          ...
          if (typeof outboundQ === "number")
            updateStopCount(
              d.stop_id,
              outboundQ,
              typeof inboundQ === "number" ? inboundQ : undefined
            );
  ...
  async function postSpeed(sp: number) {
    if (!connId) return;
    const payload = { conn_id: connId, speed: sp } as const;
    try {
